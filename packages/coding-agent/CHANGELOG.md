...
- `omp gc --merge-sessions` now retires a fork whose entries the parent already has. The pass used to stop at "this fork contributes nothing" and leave the file in place, but nothing-left-to-add is the state a completed merge produces: the content is in the parent and the fork file is a second copy of one conversation in the session list, permanently. Such a fork is archived with its artifacts, and the parent is not rewritten — no graft to make, so no backup churn on a large session.
