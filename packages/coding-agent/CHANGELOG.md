...
- Added `omp gc --prune-empty-sessions archive|delete` for whole conversations the model never answered — the session-level counterpart to `/prune`. A session counts as answered if any assistant message carries text or ended its turn normally, so a run full of tool calls that was abandoned before a reply qualifies while a session where the model said anything at all does not. Archiving is the recommended mode; the live session is never touched.
