...
describe("tree selector collapse", () => {
	...
	it("jumps between fork points with left and right", () => {
		const selector = selectorAt("a3"); // rows: root, a1, a2, a3, b1 — root forks
		const list = selector.getTreeList();

		selector.handleInput(LEFT);
		expect(list.getSelectedNode()?.entry.id).toBe("root");

		// No fork below the cursor, so right falls through to the last row.
		selector.handleInput(RIGHT);
		expect(list.getSelectedNode()?.entry.id).toBe("b1");
	});
