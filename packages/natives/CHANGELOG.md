...
- Fixed Jujutsu `statusSummary` and `statusPorcelain` reading a stale working-copy commit, so uncommitted and untracked files now appear without waiting for another command to snapshot.
